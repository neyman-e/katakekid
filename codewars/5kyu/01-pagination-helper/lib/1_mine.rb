...
# https://www.codewars.com/kata/515bb423de843ea99400000a/ruby
...
class PaginationHelper
  ...
  def page_count
    @collection.size.fdiv(@items_per_page).ceil
  end

  # returns the number of items on the current page. page_index is zero based.
  # this method should return -1 for page_index values that are out of range
  def page_item_count(page)
    return -1 if page < 0 or page >= @pages.size
    @pages[page].size
  end
  ...
  def page_index(item_index)
    return -1 if item_index < 0 or item_index >= @collection.size
...
p helper.page_index(4) # should == 1 (zero based index)
